                        ...
                        >
                            <DialogPanel className="w-full max-w-6xl transform overflow-hidden rounded-3xl bg-white/5 text-left align-middle shadow-2xl border border-gray-800 transition-all font-inter text-white relative p-6 space-y-4">
                                ...
                                <div className="flex flex-col sm:flex-row items-center gap-4 justify-center">
                                    ...
                                    <button
                                        onClick={() => type == "new" ? createSubscription.mutate({ tier: selectedTier!.tierName, planPDA, payerKey: publicKey!, periodSeconds: Number(selectedTier?.periodSeconds), amount: Number(selectedTier?.amount), autoRenew, receiver: new PublicKey(processedPlan.receiver), mint: new PublicKey(processedPlan.mint) }) : managePlan.mutate({ subscriptionPDA: subscriptionPDA!, field: "tier", value: selectedTier?.tierName!, payerKey: subscriptionPayer! })}
